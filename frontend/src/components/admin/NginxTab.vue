...
<template>
  <div class="nginx-tab">
    <h3 class="section-title">🌐 域名解析（Nginx）</h3>
    <p class="form-sub">将特定域名反向代理到本系统，仅超管可配置。</p>
    ...
    <template v-else>
      ...
      <div class="setting-item">
        <div class="setting-info">
          ...
          <p class="setting-desc">将该域名反向代理到本系统端口 {{ st.port }}。格式如 wall.example.com</p>
        </div>
        <div class="nginx-form">
          <input v-model="form.domain" class="input" placeholder="域名，如 wall.example.com" :disabled="!st.is_server || busy" />
        ...
        <p class="form-tip">同时填证书和私钥则启用 HTTPS；留空为纯 HTTP。</p>
      </div>
