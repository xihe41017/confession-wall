...
- 数据源：JSON 文件（/etc/campus/confession-domains.json），不是解析 Nginx 配置本身
...
# ---------- 每系统常量（radio 项目需同步修改） ----------
SYSTEM_NAME = "confession"
SERVICE_NAME = "campus-confession"
DEFAULT_PORT = 8000
CLIENT_MAX_BODY = "60m"   # 表白墙支持上传图片/视频，放宽请求体

# 域名存储文件（后台管理的数据源）
DOMAINS_FILE = os.getenv("NGINX_DOMAINS_FILE", "/etc/campus/confession-domains.json")

# 生成的 Nginx 配置（apt 用 sites-available + 软链，yum 用 conf.d）
CONF_FILE = os.getenv("NGINX_CONF_FILE", "/etc/nginx/conf.d/campus-confession-domains.conf")
...
def add_domain(domain: str, ssl_cert: str = "", ssl_key: str = "") -> tuple:
    ...
    if not _DOMAIN_RE.match(domain):
        return False, "域名格式不正确（示例：wall.example.com）"
    domains = _load()
